...
class PerfectComputerPlayer < PerfectPlayer 
    def move(game, mark)
        puts "Just a moment...\n\n"
    ...
    def minimax(board, mark)
        successor_function = Pro.new do |board|
            generate_child_nodes(board)
        end

        evaluation_fuction = Pro.new do |board|
        ...
        MinimaxNode.set_evaluation_function(evaluation_fuction)
